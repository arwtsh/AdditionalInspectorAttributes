...
namespace ConditionalInspector
{
    ...
    internal class ShowInInspectorIfTrueDrawer : HideInInspectorIfDrawer
    {
        protected override bool IsVisible(SerializedObject serializedObject)
        {
            ShowInInspectorIfTrueAttribute conditional = (ShowInInspectorIfTrueAttribute)attribute;
            ...
            if (propertyReference != null)
            {
                if (propertyReference.PropertyType != typeof(bool))
                {
                    Debug.LogWarning("The property " + propertyReference.Name + " in type " + fieldInfo.DeclaringType + " is type " + propertyReference.PropertyType.Name + " instead of bool. " +
                        "The ShowInInspectorIfTrue attribute on property " + fieldInfo.Name + " in type " + fieldInfo.DeclaringType + " was unable to be parsed.");
                    return true;
                }
            ...
            Debug.LogWarning("The string passed into attribute ShowInInspectorIfTrue on property " + fieldInfo.Name + " in type " + fieldInfo.DeclaringType + " does not match any property, field, or method.");
            return true;
        }
    }
}
