...
namespace ConditionalInspector
{
    ...
    public class ShowInInspectorIfNullDrawer : HideInInspectorIfDrawer
    {
        protected override bool IsVisible(SerializedObject serializedObject)
        {
            ShowInInspectorIfNullAttribute conditional = (ShowInInspectorIfNullAttribute)attribute;

            if (System.String.IsNullOrEmpty(conditional.propertyName))
            {
                Debug.LogWarning("The string passed into attribute ShowInInspectorIfNull on property " + fieldInfo.Name + " in type " + fieldInfo.DeclaringType + " was null or empty.");
                return true;
            }
            ...
            if (propertyReference != null)
            {
                if (!Utilities.IsTypeNullable(propertyReference.PropertyType))
                {
                    Debug.LogWarning("The property " + propertyReference.Name + " in type " + fieldInfo.DeclaringType + " is type " + propertyReference.PropertyType.Name + ", which isn't nullable. " +
                        "The ShowInInspectorIfNull attribute on property " + fieldInfo.Name + " in type " + fieldInfo.DeclaringType + " was unable to be parsed.");
                    return true;
                }
            ...
            Debug.LogWarning("The string passed into attribute ShowInInspectorIfNull on property " + fieldInfo.Name + " in type " + fieldInfo.DeclaringType + " does not match any property, field, or method.");
            return true;
        }
    }
}
