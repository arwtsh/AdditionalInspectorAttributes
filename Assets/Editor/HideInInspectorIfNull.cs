...
namespace ConditionalInspector
{
    ...
    /// </summary>
    public class HideInInspectorIfNullAttribute : PropertyAttribute
    {
    ...
    [CustomPropertyDrawer(typeof(HideInInspectorIfNullAttribute))]
    internal class HideInInspectorIfNullDrawer : HideInInspectorIfDrawer
    {
        protected override bool IsVisible(SerializedObject serializedObject)
        {
            HideInInspectorIfNullAttribute conditional = (HideInInspectorIfNullAttribute)attribute;

            if (System.String.IsNullOrEmpty(conditional.propertyName))
            {
                Debug.LogWarning("The string passed into attribute HideInInspectorIfNull on property " + fieldInfo.Name + " in type " + fieldInfo.DeclaringType + " was null or empty.");
                return true;
            }
            ...
            if (propertyReference != null)
            {
                if (!Utilities.IsTypeNullable(propertyReference.PropertyType))
                {
                    Debug.LogWarning("The property " + propertyReference.Name + " in type " + fieldInfo.DeclaringType + " is type " + propertyReference.PropertyType.Name + ", which isn't nullable. " +
                        "The HideInInspectorIfNull attribute on property " + fieldInfo.Name + " in type " + fieldInfo.DeclaringType + " was unable to be parsed.");
                    return true;
                }
            ...
            Debug.LogWarning("The string passed into attribute HideInInspectorIfNull on property " + fieldInfo.Name + " in type " + fieldInfo.DeclaringType + " does not match any property, field, or method.");
            return true;
        }
    }
}
